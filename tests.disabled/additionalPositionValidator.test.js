...
const { validateCheckAdditionalPosition, validateFilterAdditionalPositions } = require('../../../src/validators/additionalPositionValidator');

describe.skip('Additional Position Validators', () => {
  describe('validateCheckAdditionalPosition', () => {
    it('should fail validation with missing unique_id', async () => {
      const req = { body: {} };
      const res = {};
      const next = jest.fn();

      await validateCheckAdditionalPosition(req, res, next);
  ...
  describe('validateFilterAdditionalPositions', () => {
    it('should fail validation with invalid date format', async () => {
      const req = { body: { date: 'invalid-date' } };
      const res = {};
      const next = jest.fn();

      await validateFilterAdditionalPositions(req, res, next);
